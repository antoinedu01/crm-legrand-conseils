// Tests de la migration v9 (table `appointments`, Acquisition OS, lot A2a).
// Même convention que test/migrations.test.js (v8) : base SQLite temporaire
...
// Reconstruction fidèle du schéma pré-v9 (base + migrations v1 à v8),
...
// existe déjà, quelle que soit la version de départ.
...
test('migration v9 — une base héritée en v8 est migrée vers v9 sans perte de données (scénarios 1, 2, 13)', async () => {
  const dir = tempDir();
...
test('migration v9 — la table appointments existe avec les colonnes attendues (scénarios 3, 4)', async () => {
  const db = await importFreshDb(tempDir());
  const exists = db
...
test('migration v9 — création d’un rendez-vous valide, plusieurs rendez-vous pour un même client (scénarios 5, 6, 7)', async () => {
  const db = await importFreshDb(tempDir());
  const client = db
...
test('migration v9 — un client_id inexistant est rejeté par la contrainte FK (scénario 8)', async () => {
  const db = await importFreshDb(tempDir());
  assert.throws(
...
test('migration v9 — status vaut « booked » par défaut si non renseigné (scénario 9)', async () => {
  const db = await importFreshDb(tempDir());
  const client = db
...
// -- 10 — indexes attendus -------------------------------------------------
test('migration v9 — les index attendus existent (scénario 10)', async () => {
  const db = await importFreshDb(tempDir());
  const indexes = db
...
// -- 11 — idempotence -------------------------------------------------------
test("migration v9 — idempotence : un second import de la même base n'échoue pas et reste en v9 (scénario 11)", async () => {
  const dir = tempDir();
...
test('migration v9 — les tables existantes (v1 à v8) restent toutes présentes (scénario 12)', async () => {
  const db = await importFreshDb(tempDir());
  const tables = db
  ...
  for (const name of expected) {
    assert.ok(tables.includes(name), `table ${name} manquante après migration v9`);
  }
});

// -- Contrainte CHECK réellement ajoutée (ends_at > starts_at) --------------
test('migration v9 — la contrainte CHECK ends_at > starts_at fonctionne', async () => {
  const db = await importFreshDb(tempDir());
  const client = db
...
test('migration v9 — aucun CHECK SQL sur status/appointment_type/location_type (validation déléguée à l’API, comme le reste du schéma)', async () => {
  const db = await importFreshDb(tempDir());
  const client = db
