...
// Canonicalisation récursive d'un objet JSON (condition/validation) : trie
// les clés d'un objet par ordre alphabétique, conserve l'ordre des tableaux
// (fonctionnellement significatif, ex. une liste de valeurs `in`), n'exécute
// jamais aucun code, ne modifie jamais le contenu stocké en base — ce n'est
// qu'une vue transitoire utilisée uniquement pour le calcul de l'empreinte.
// Aucune dépendance externe : fonction locale minimale.
//
...
// (constat vérifié empiriquement pendant le GATE, corrigé par prudence bien
// qu'aucun schéma actuel n'autorise une telle clé — `validateConditionFormat`
// restreint déjà strictement les clés acceptées).
