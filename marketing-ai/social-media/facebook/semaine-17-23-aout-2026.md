# Facebook — Semaine du 17 au 23 août 2026

> **Version finale — prête pour validation humaine** (finalisée le 17/07/2026).
> Diffusion envisagée le 22/08/2026. **Page Facebook créée — URL publique à
> confirmer** (ne jamais l'inventer). Le contenu suit le circuit normal :
> validation humaine + confirmation de l'URL de la page Contact (CTA) avant
> programmation manuelle. Voir `../checklist-creation-pages-linkedin-facebook.md`.
> Producteur : `social-media-manager` · Contrôle : `compliance-reviewer`.
> Legrand conseils Sàrl — Suisse romande (Vaud / Genève).
> Contenu **réellement adapté à Facebook** (ton proximité, communauté locale) —
> ce n'est ni le post LinkedIn, ni le carrousel Instagram.

---

## Publication — Samedi 22 août 2026

- **Date proposée** : samedi 22/08/2026 (fin de matinée).
- **Persona** : B — Couple / famille.
- **Pilier éditorial** : P6 — Protéger son couple ou sa famille.
- **Objectif** : éducation + prise de contact volontaire (page Contact).

### Texte complet

L'arrivée d'un enfant, c'est mille choses à penser en même temps. 🍼

Entre les nuits courtes et la logistique, les questions d'assurance passent souvent (logiquement) au second plan. Pourtant, une naissance change pas mal de choses côté couverture du foyer.

Sans rentrer dans les détails techniques, voici trois points qu'il vaut la peine de vérifier une fois que le tourbillon des premières semaines est un peu retombé :

1️⃣ La couverture maladie du nouveau-né — pour s'assurer que tout est bien en ordre dès le départ.

2️⃣ La protection du foyer si l'un des deux parents ne pouvait plus travailler durablement — un sujet qu'on préfère ne pas imaginer, mais qui compte quand on devient responsable d'un enfant.

3️⃣ La cohérence de l'ensemble — parce qu'avec le temps, on accumule des contrats un peu partout, et il arrive qu'on paie en double… ou qu'il manque quelque chose.

Rien d'urgent : juste un petit point à faire, tranquillement, quand vous serez prêts.

Avec Legrand conseils Sàrl, j'accompagne les familles de Suisse romande pour y voir clair, sans pression et à leur rythme.

- **Appel à l'action** : « Vous souhaitez faire le point sur votre situation ? Vous pouvez présenter votre demande depuis la page Contact de Legrand conseils Sàrl. »
  - *(La « check-list du foyer » n'existe pas encore — elle est prévue en Phase 3. Elle n'est donc **pas** proposée ici comme ressource disponible.)*
- **Page du site envisagée** : page **Contact** (documentée comme existante dans le calendrier LOT 1). `URL de la page Contact à confirmer avant publication`.
- **Visuel suggéré** : photo chaleureuse et sobre (parent et bébé, ambiance douce), sans mise en scène commerciale ; ou visuel « 3 points » aux couleurs de la marque.
- **Sources utilisées** : notions générales (couverture maladie du nouveau-né, protection du revenu du foyer, risque de doublons/lacunes). *Aucune donnée chiffrée.*
- **Affirmations à vérifier** :
  - Modalités et délais d'affiliation maladie d'un nouveau-né en Suisse → source officielle (OFSP). `Vérification humaine obligatoire`.
  - Ne pas introduire de délai ou de montant sans source. `Vérification humaine obligatoire`.
- **Dépendance signalée** : la ressource « check-list du foyer » (lead magnet 2) **n'est pas encore produite** (prévue en Phase 3). Le CTA a été révisé pour renvoyer vers la **page Contact** (existante) et **ne présente plus** de ressource indisponible.
- **KPI principal** : portée, engagement, clics vers la page Contact.
- **Statut** : `Version finale — prête pour validation humaine`.

---

### Notes de production (Facebook)
- Ton plus chaleureux et local que LinkedIn ; pas de jargon.
- Aucune promesse d'économie/amélioration de couverture ; pas de « meilleur produit ».
- Sujet sensible (famille, naissance) traité **sans alarmisme** et **sans collecte
  de données de santé**.
